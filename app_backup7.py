...
def page_coach_student_history():
    ...
    if weights:
        # 準備體重數據
        weight_xs = [r.get("timestamp", "")[:10] for r in sorted_w]
        weight_ys = [r.get("weight_kg", 0) for r in sorted_w]
        
        max_weight = max(weight_ys) if weight_ys else 0
        min_weight = min(weight_ys) if weight_ys else 0
        weight_range = max_weight - min_weight
        weight_ticks = []
        if weight_range > 0:
            # 根據體重範圍動態調整刻度
            step = max(1, int(weight_range / 4))
            start = int(min_weight) - step
            end = int(max_weight) + step
            weight_ticks = list(range(start, end + 1, step))
        ...
        fig_weight.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor='rgba(255, 255, 255, 0.05)', tickfont=dict(color='#888888', size=11), zeroline=False, showline=False, ticks="", tickvals=weight_ticks if weight_ticks else None),
            showlegend=False
        )
