...
carb, fat, water_ml, image_url, portion, record_id
```

- 新紀錄種類只應為「食物」或「飲水」。
- `image_url` 固定留空。
- 同日多筆飲水與食物由查詢端加總。
- `record_id` 是學員修改或刪除單筆紀錄的穩定識別值。

### Weight

```text
timestamp, user_id, weight_kg, record_id
```

- 新資料使用完整 ISO timestamp。
- 舊 date-only 資料仍可讀取。
- 同一時間戳若重複，工作表較後列視為較新。
- 新增、修改與刪除以 `record_id` 精準定位，timestamp 僅供排序與舊流程相容。
...
目前完整測試基準為 `248 passed`。新增功能時至少要補足純函式、服務層與主要頁面路由測試。
...
先部署可相容舊 schema 的程式，再執行：

```powershell
python tools/migrate_record_ids.py
python tools/migrate_record_ids.py --apply
```

預覽會列出 Records、Weight、Training 的筆數、空白 ID 與重複 ID，不會寫入資料。`--apply` 會先將三張工作表保存至 `backups/record_ids_<時間>/`，再於尾端新增 `record_id` 並只回填空白值；遇到未知 schema 或重複 ID 時會在寫入前停止。
